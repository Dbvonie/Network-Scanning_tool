...
_LA_BIT = 0x02  # Locally Administered bit (bit 1 of first octet)


def _is_private_mac(mac: str) -> bool:
    """
    Returns True for locally administered (randomised / privacy) MACs.

    The locally-administered bit is bit 1 of the first octet (_LA_BIT).
    Private MACs generated by iOS, Android, Windows 10+ privacy mode, etc.
    all set this bit.
    """
    try:
        first_byte = int(mac[:2], 16)
        return bool(first_byte & _LA_BIT)
...
def mac_to_vendor(mac: str) -> str:
    ...
    Retourne "Private/Randomized" pour les MACs administrés localement
    (bit LA=1), "Unknown" si pas trouvé.

    Args:
        mac: Adresse MAC normalisée (AA:BB:CC:DD:EE:FF).

    Returns:
        Nom du vendor, "Private/Randomized", ou "Unknown".
    """
    if not mac or len(mac) < 8:
        return "Unknown"

    # Locally administered MACs are privacy/random — do not guess vendor
    ...
    # Exact OUI match only — the 5-char prefix fallback caused false positives
    # (e.g. "AA:BB:CC" incorrectly resolving to a vendor for "AA:BB:DD").
